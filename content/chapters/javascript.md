...
При обновлении до v190 все десять функциональных JS-модулей совпали по SHA-256 с ранее разобранной версией. Изменения importmap связаны с адресом версии. Модули — браузерные ESM; React, сборщик и Node.js для них не нужны. Подключайте только нужные функции.

## Подключение из latest

Карта зависит от канала: `latest/importmap.js` указывает на `latest`, а версионный importmap — на собственную версию. Штатный загрузчик подключайте до первого ESM. Для явного контроля адресов можно оставить карту ниже:
...
Версия, дата и хеши источников — в [манифесте](../reference/source-manifest.json). Runtime использует изменяемый `latest`. Минифицированные файлы проверены по SHA-256; их семантическая эквивалентность неминифицированным не заявляется. Копии файлов в репозитории не хранятся.
